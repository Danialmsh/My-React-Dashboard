...
const SidebarGroupTitle = ({ title, pTitles, icon, children }) => {
    const [open, setOpen] = useState(false);
    const hasPerm = useHasPermission(pTitles);
    const collapseRef = useRef(null);

    useEffect(() => {
        // بررسی وجود آیتم active در children
        if (collapseRef.current) {
            const hasActive = collapseRef.current.querySelector('.sidebar_menu_item.active');
            if (hasActive) {
                setOpen(true); // اگر آیتم active بود، منو رو باز کن
            }
        }
    }, [children]);
    ...
    return hasPerm && (
        <>
            ...
            >
                ...
                <span className="hiddenable no_wrap group_sidebar_title ms-2 font_09">{title}</span>
                {open ? (
            ...
            <Collapse in={open}>
                <div id="example-collapse-text" ref={collapseRef}>
                    {children}
                </div>
            </Collapse>
        </>
